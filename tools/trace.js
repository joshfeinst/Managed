/* Managed: play-trace. Runs seeded careers through the REAL systems and emits
   ...
   Usage: node tools/trace.js /abs/path/index.html [careers=3] [skill=0.75] */
const { chromium } = require('playwright');
const fs = require('fs'), path = require('path');
const target = process.argv[2];
const N = +(process.argv[3] || 3);
const SKILL = +(process.argv[4] || 0.75);
const PICK = +(process.argv[5] || 0);   // which dialogue option the player favours
...
(async () => {
  ...
  const out = await page.evaluate(({ N, SKILL, PICK }) => {
    ...
    for (let c = 0; c < N; c++){
      ...
      while (run && guard++ < 60){
        ...
        simDay(SKILL, { pick: PICK });
        if (!run){ break; }
        const dayLog = SLOG.slice(mark);
        ...
        for (const l of dayLog){
          const m = l.match(/resolved (\d+)%: (.*?)(?: \(craft|$)/);
          if (m){ const t = m[2].slice(0,42); agg.ticketSeen[t]=(agg.ticketSeen[t]||0)+1;
                  (agg.ticketScore[t] = agg.ticketScore[t] || []).push(+m[1]); }
          const b = l.match(/BREACHED: (.*)/); if (b) agg.breaches += 0;
  ...
  console.log('=== MANAGED PLAY TRACE ===  careers=' + N + ' skill=' + SKILL + ' choicePolicy=option' + (PICK+1));
